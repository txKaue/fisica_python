...
def add_object(space,radius,pos):
    body = pymunk.Body(body_type=pymunk.Body.STATIC)
    body.position = (300,150) #posição
...
def run(window, widht, height):
    ...
    space = pymunk.Space()
    space.gravity = (0, 981) #isso aqui é a gravidade

    createboundaries(space, widht, height)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if not ball:
                    ...
                    ball = add_object(space, 30, pressed_pos)
                elif pressed_pos:
                    ball.body.body_type = pymunk.Body.DYNAMIC
                    angle = calculate_angle(*line)
                    force = calculate_distance(*line) * 10
